...
def explain_match(catalog, ratings, title, genre=None, fatigue_overrides=None, top_n=5, format_preference=None):
    ...
    # Validated here, before resolving anything, so an unknown title
    # still fails fast and cheap -- CODX's Task 13 review (2026-09-19)
    # caught that the extraction below had silently moved this check
    # to AFTER profile resolution, so a bad title started paying full
    # resolution cost (and printing _resolve_profile's own "not found
    # in catalog" warning) before raising the exact same ValueError.
    if title not in {b["title"] for b in catalog.values()}:
        raise ValueError(f"{title!r} not found in catalog")
