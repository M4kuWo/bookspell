...
for line in task.splitlines():
    if line.startswith('| ') and re.search(r'\| 0\.[34] \|$', line):
        title, author, field, assigned_value, confidence = [s.strip() for s in line.strip('|').split('|')]
        pairs.append(dict(title=title, author=author, field=field, assigned_value=assigned_value, assigned_confidence=float(confidence)))
assert len(pairs) == 22
books = []
while True:
    page = get('books', dict(select='id,title,author,hardcover_id,isbn,publication_year,synopsis,series_id,archived', order='id', limit=1000, offset=len(books)))
    books.extend(page)
    if len(page) < 1000:
        break
selected = []
